...
class Lithic::Test::PrimitiveModelTest < Minitest::Test
  ...
  def test_coerce
    ...
    cases.each do |lhs, rhs|
      ...
      assert_pattern do
        Lithic::Type::Converter.coerce(target, input, state: state) => ^expect
  ...
  def test_dump
    ...
    cases.each do
      ...
      expect = _2
      assert_pattern do
        Lithic::Type::Converter.dump(target, input) => ^expect
      end
    end
  end
  ...
  def test_coerce_errors
    ...
    cases.each do
      ...
      assert_raises(_2) do
        Lithic::Type::Converter.coerce(target, input, state: state)
      end
    end
  end
end
...
class Lithic::Test::BaseModelTest < Minitest::Test
  ...
  def test_coerce
    ...
    cases.each do |lhs, rhs|
      ...
      state = {strictness: true, exactness: {yes: 0, no: 0, maybe: 0}, branched: 0}
      assert_pattern do
        coerced = Lithic::Type::Converter.coerce(target, input, state: state)
